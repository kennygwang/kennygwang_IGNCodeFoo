var calcPlates = function() {
    ...
    x--;                                      // above loop found least power of 10 greater than target, minus one is greatest power smaller than the target
    ...
    if(x >= 0 && [Math.abs([Math.pow(10,x)*Math.pow(26,y) - population]) <= Math.abs([Math.pow(26,a)*Math.pow(10,b) - population])]) {
        nDigits = x;
        nLetters = y;
